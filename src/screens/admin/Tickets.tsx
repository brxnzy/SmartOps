  ...
  return (
    <section className="space-y-6">
      <header className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">Gestiona las averias reportadas.</p>
      </header>
      ...
      <div className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            placeholder="Buscar por codigo, cliente o categoria"
                ...
                {filtered.map((ticket) => (
                  <tr key={ticket.id} className="hover:bg-slate-50/80">
                    ...
                    <td className="px-4 py-3 text-slate-600">
                      {ticket.technicianName ?? (ticket.assignedTo ? ticket.assignedTo.slice(0, 8) : "Sin asignar")}
                    </td>
