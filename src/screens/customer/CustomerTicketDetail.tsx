...
export default function CustomerTicketDetail() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    getTicketDetail(companyId, ticketId)
      .then((data) => {
        if (!active) return;
        setTicket(data.ticket);
        setComments(data.comments.filter((comment) => !comment.isInternal));
        setAttachments(
          data.attachments
  ...
  const handleCommentSubmit = async () => {
    if (!companyId || !customerId || !ticketId) return;
    ...
    try {
      await addTicketComment(
        ...
        ticketId,
        customerId,
        { body: newComment.trim(), isInternal: false },
        commentFiles
      );
      const updated = await getTicketDetail(companyId, ticketId);
      setComments(updated.comments.filter((comment) => !comment.isInternal));
      setNewComment("");
      setCommentFiles([]);
    } catch (err) {
    ...
    <section className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-sm font-semibold text-slate-900">{ticket.slaType}</p>
